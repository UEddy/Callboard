...
- **The organiser sign-in prints the magic link on screen by default.** Anyone who can reach `/admin/sign-in` and knows, or simply reads, an organiser's address can sign in as them. That is a deliberate trade so a fresh deployment can be opened without an inbox, and it is one setting away from closed: `ADMIN_LINK_ON_SCREEN: "off"` in `wrangler.jsonc`. A self hoster running a real call for speakers should change it on day one.
- **`is_admin` is a single level of access.** An organiser can do everything: edit evaluation plans, commit decisions, read every score. There are no per area roles, so a volunteer who only needs to chase headshots has the same reach as the programme director.
