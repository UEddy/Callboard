...
**Chosen access method: the on-screen link, not a shared passphrase.** A passphrase would have to be set as a secret on every deployment, and a deployment where nobody set it is a deployment nobody can administer. The on-screen link needs no configuration, is bounded to accounts that already have `is_admin` set, and reuses the same one-shot token machinery as the speaker portal, so it is not a second, weaker way in.

It is on by default because being locked out of `/admin` is worse than the exposure for a demo. On a deployment holding real submissions, set `ADMIN_LINK_ON_SCREEN` to `off` in `wrangler.jsonc`: the link is then emailed and nothing else, the organiser addresses stop being listed on the page, and knowing an organiser's address is no longer enough to sign in as them. Nothing else about the flow changes.
...
Everything a rule does is recorded on the submission as a routing trail and shown on the submission detail page, in plain language: `Because Format is "Workshop (90 min)": sent to Workshop Review with 2 reviewers assigned`.

A producer can edit the title, description, format, level, track and tags from that page, and every edit joins the same trail, so the panel is "What happened to this" rather than routing only: who changed what, from what to what, and when. There is no admin sign-in in Callboard, so the editor names themselves from the event's admins, the same convention the evaluation screen uses for "Reviewing as", and an id that is not one of them is refused rather than written into the history as fact.

Editing deliberately does not re-run routing. A rule fired once, at submit time, on what the submitter wrote; a producer changing a format afterwards is making a decision, and having that decision silently reassign the review committee would be the opposite of one. The rules are evaluated against the new values anyway, and what they would have done is reported on screen and recorded in the history as declined: "Now that Format is 'Workshop (90 min)', a routing rule would have sent it to Workshop Review with 2 reviewers. Editing does not re-run routing, so none of that happened." A description written with formatting says so before it is edited, because the box is plain text and losing somebody's markup silently would be worse than not offering the edit.
...
Round robin fills a programme; assigning by hand is for "she should look at these two". Both ends of that are covered. The Evaluators tab has an Assign action per reviewer: a searchable list of the event's submissions, drafts and withdrawn ones left out, with anything already on their plate and anything they have a conflict of interest with shown but unpickable and labelled with which it is. Hiding those two outright would only prompt a producer to go looking for the missing submission again tomorrow. The submission detail page has the inverse, so somebody reading a proposal can staff it without going to find each reviewer: the evaluators listed lightest load first with their assigned and completed counts, conflicts marked, and anybody already on it marked too.

Both go through one implementation in `app/lib/evaluation.ts`, which re-checks the conflict and the duplicate on the server before inserting anything. A control that only prevents a thing in the markup prevents nothing, and the result says what it did: "1 assignment created, 1 skipped for a conflict of interest."
...
A matrix of accepted speakers against onboarding tasks (headshot, bio, slides, recording release, travel confirmation), one coloured dot per cell, sorted worst first: most overdue items, then least complete, then alphabetically. Filter chips narrow it to incomplete, overdue or complete, and a search box narrows it by speaker name, company or address. Both are in the URL, and both apply to the counters above the matrix as well as the matrix itself: a producer who has filtered to the overdue speakers is asking about those speakers, and a header still reporting the whole event would be answering a question nobody asked. The chip counts are taken over the searched set, so they describe the board in front of you. Four counters across the top: accepted speakers, fully onboarded, speakers with overdue items, and total open items. Accepting a submission is what creates the task assignments, so the dashboard fills itself as decisions go out.
...
An attendee can build a personal schedule without an account. Every session card, agenda block and itinerary row carries a star, and the starred refs live in `localStorage` keyed by event slug, so two conferences on one deployment do not share a list and nothing is sent to the server: the public layer is anonymous and cached at the edge, and what somebody starred is nobody's business but theirs. Storage is read after mount rather than during render, because the server has no `localStorage` and a star drawn from it on the first pass would be a hydration mismatch. A My schedule view filters to the starred sessions, in time order, and the view switcher carries the count. It is not an embeddable format, since there is nothing personal for a producer to publish, but it works inside `?embed=1` like everything else.

Two calendar downloads come out of the same ICS builder the acceptance invitations use, extended with a PUBLISH calendar that can hold many events: one session from its detail page, and the whole personal schedule as a multi-event file. The refs travel in the URL (`/e/:eventSlug/calendar.ics?s=SESS-1&s=SESS-4`), which is what lets a personal schedule download without an account, and the route reads through the same loader as the public views so it can never export something the programme does not already show. Times go out as UTC instants, which every calendar client renders in the reader's own zone, with the event's local time repeated in the description alongside the room, the track and the speakers, because a phone notification shows the description and somebody standing in a corridor wants the room.
...
The organiser sign-in at `/admin/sign-in` deliberately does not make that promise, and says so rather than pretending otherwise. It tells you when an address is not an organiser account, because a page that prints the link on screen for organiser addresses already reveals which ones they are, and a mistyped address failing silently would be worse than useless. Turning `ADMIN_LINK_ON_SCREEN` off removes both the printed link and the listed addresses together, since they are the same disclosure.

### Organisers sign in separately from speakers

`/admin` is gated by an `is_admin` flag on the participant, and organiser sessions are a different cookie (`cb_admin`) from portal sessions (`cb_portal`), signed with a different secret and valid for twelve hours rather than sixty days. A speaker who signs into the portal gets a portal cookie and nothing else. An organiser can hold both at once and neither implies the other. The two magic links are separated in the token table as well, by a `purpose` column: pasting a portal link into the organiser sign-in is refused, and refused without burning it, so a speaker's link still works afterwards.

The flag is re-read on every request rather than trusted from sign-in time, so clearing `is_admin` locks somebody out on their next click instead of whenever their cookie happens to expire.

Why the gate lives in the worker's fetch handler rather than in the admin layout's loader: a child route's action runs without its parent's loader ever being called. A loader-only check looks protective and leaves every `POST` under `/admin` open, creating people, committing decisions, deleting rooms. Checking before the router runs covers pages, data requests, form posts, CSV downloads, and any route added later, with no way to forget one. The cost is that the check cannot use route context, which is why it builds its own database handle.

That placement has one consequence worth naming. A client-side navigation asks for `/admin/people.data` and expects a turbo-stream body; an ordinary 302 gets followed by `fetch`, returns the sign-in page's HTML, and the router dies decoding it, which is exactly what an expired session in an open tab would look like. Data requests therefore get a `204` with `X-Remix-Redirect`, which React Router turns back into a real navigation, and the sign-in page says the session ended rather than showing a stack trace.
...
The Evaluators tab chases reviewers the same way, from the same rules in `app/lib/nudge.ts` so the two boards cannot disagree about what "2d ago" or "too recently" means: a Remind per reviewer and a Remind all for everyone with incomplete reviews, an email saying how many submissions are still waiting with a link straight into their queue, a timestamp on their open assignments only, and the same amber cooldown warning. A reviewer with nothing outstanding shows "Nothing outstanding" instead of a button.
...
## Scope we decided not to build

These are not oversights. Each one was considered, and each was cut for a reason worth stating, because a tool that does eight things properly is worth more than one that half does fifteen.

- **Multi round evaluation.** One scoring pass per plan. Real committees often do a screening round then a deeper read of what survives, which needs rounds as a first class object: per round criteria, per round assignment, and a rule for carrying scores forward. Building it half way, as a "round 2" flag on an assignment, produces aggregate scores that silently mix two different questions. A single round that is honest about being one beats a second round that corrupts the first.
- **File versioning.** An upload replaces what was there. Speakers send v2 of a deck the night before and organisers want the old one back, so this is a genuine gap, but it is a storage and retention design (how many versions, for how long, who can see superseded ones, what R2 costs) rather than an afternoon's work.
- **Multi event.** Every table carries an `event_id` and the schema would take a second event today, but the app resolves a single demo event id, and the public layer, exports and settings all assume one. Making it real means an event switcher, per event access control, and deciding what the roster means across events. That is a product, not a feature.
- **CSV import.** Bulk loading speakers or an existing programme from a spreadsheet. Import is where data quality goes to die: column mapping, partial failures, duplicate detection against existing rows, and a preview that shows what will happen before it happens. Done badly it is worse than typing, and there is a working API for anyone who needs to move data in bulk.
- **ZIP export of files.** Downloading every headshot and deck as one archive. Exports go out as CSV and XLSX across six datasets, which covers the reporting need; bundling binaries means streaming a large archive out of R2 inside a Worker's limits, which is a different problem from writing a spreadsheet.
- **General change history.** Submissions have an edit trail: who changed which field, when, and what the value was, shown in the routing panel alongside the automated entries. That is deliberately scoped to the one place where "who changed the title" gets asked. There is no audit log across the rest of the app, so a room rename or a settings change leaves no record. A real audit log is a cross cutting concern that wants a single write path for every mutation, and retrofitting one late tends to produce a log that is trusted more than it deserves to be.
- **Automatic schedule generation.** The agenda detects conflicts but never resolves them. Constraint solving a conference (room capacity, track spread, speaker availability, keynote placement) produces a grid no human agrees with, and organisers rearrange it anyway. Detection is the part that repays automation; placement is judgement.
- **Speaker CRM.** No history of past events, no relationship notes, no invited speaker pipeline. The roster is scoped to this event's participants, which keeps it a working tool for running one programme rather than a database of people to court.

## Known limitations

Stated plainly, because a judge will find them anyway.

- **Organiser sign-in is open to any address, and there is no allowlist.** Enter any email at the root and you get a working link; an address nobody has seen before becomes a participant on the spot. The `is_admin` column still exists and still marks the seeded organisers, but nothing gates on it. This is a deliberate demo affordance so that somebody evaluating the app can use their own inbox without being added to anything first, and it means **anyone who can reach the site can reach the organiser area**. A real deployment would restore the check in `adminFromRequest` and in the sign-in action, which are the only two places it was removed from.
- **The sign-in page prints the magic link on screen.** Combined with the point above, reaching the page is enough to sign in; no inbox required. Same reason, same trade, and one setting away from closed: `ADMIN_LINK_ON_SCREEN: "off"` in `wrangler.jsonc`. With it off, links are emailed and nothing else, and the seeded organiser addresses stop being listed on the page. A self hoster running a real call for speakers should change both on day one.
- **There is one level of organiser access.** Anyone signed in can do everything: edit evaluation plans, commit decisions, read every score. There are no per area roles, so a volunteer who only needs to chase headshots has the same reach as the programme director.
- **The public form does not verify the email address it is given, and finishing a submission signs you into that address's portal.** Type a real speaker's address at the account step, complete a proposal, and the success page hands you their portal: their other submissions, their tasks, their profile. The fix is to make the account step mint a magic link rather than trust the address, which is the mechanism the portal already uses.
- **Uploaded files are reachable by anyone holding the URL.** Objects are served from `/files/...` with no session check, so a slide deck is protected only by its path being hard to guess. Headshots are meant to be public; slides are not. This needs signed URLs or a session check on the serving route.
- **Nothing rate limits either magic link form.** A visitor can request links as fast as they can click, which is both a mail bill and a way to flood one inbox. It needs a per address cooldown.
- **Batch email sending is sequential.** Committing a queue of decisions loops over submissions, then over speakers within each, awaiting each Resend call in turn. Fine for the tens of emails one event needs; it will hit the Workers CPU limit on a batch of hundreds. It should be a queue.
- **The Airtable key is stored in plain text in D1.** The page masks it on screen and never sends the saved value back to the browser, but that is UI hygiene, not protection. Anyone with an organiser session can use the key. It belongs in Workers secrets or an encrypted column.
- **Airtable sync is bounded by what one request can do.** Sequential, with a gap between calls to respect Airtable's five per second limit, and capped at 2000 records a run. A few hundred sessions will be slow and a very large base will hit the CPU limit. It should be a queue.
...
- Conflicts of interest are declared by reviewers and respected by both the auto assigner and manual assignment, but nothing detects them automatically at submission time. The same company matches in the demo data were recorded by the seed.
- A Resend account in test mode can only send to its own owner. Mail to real speakers fails with a 403 until a sending domain is verified, and the failure is recorded in the email log. Nobody is locked out by it: a failed send keeps the sign-in link it was carrying on its log row, so an organiser can copy it from `/admin/emails`, and `/admin/people` can mint a fresh one for anybody at any time.
- `sent` in the email log means the provider accepted the message, not that it was delivered. Resend accepts addresses at domains that do not exist and reports them as sent.
